...
async function main() {
  console.log('Starting Prisma data.txt reseed...');

  const dataset = await parseDataset();
  if (dataset.categories.length === 0 || dataset.products.length === 0) {
    throw new Error('Parsed dataset is empty. Check data.txt format before seeding.');
  }

  console.log(
    `Parsed ${dataset.categories.length} categories and ${dataset.products.length} products from data.txt.`,
  );

  await clearTransactionalAndInventoryData();
  console.log('Cleared invoices, invoice items, credit transactions, products, and categories.');

  await seedCategories(dataset.categories);
  console.log(`Upserted ${dataset.categories.length} categories.`);

  await seedProducts(dataset.products);
  console.log(`Inserted ${dataset.products.length} products.`);

  console.log('Seed completed successfully. User data was left untouched.');
}
